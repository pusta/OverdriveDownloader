...
namespace OverdriveDownloader
{
    internal class Program
    {
        static async Task Main(string[] args)
        {
            ...
            {
                LogError("Usage: OverdriveDownloader <ODM file> [-delete]");
                return;
            }

            var odmFile = args[0];
            bool deleteFiles = args.Length == 2 && args[1].Equals("-delete", StringComparison.OrdinalIgnoreCase);
            ...
            if (deleteFiles)
            {
                LogInfo("Deleting non-m4b files...");
                foreach (var part in parts)
                {
                    var partname = part.Name + Path.GetExtension(part.Filename);
                    var partFilename = Path.Combine(odmDir, partname);
                    ...
                    if (File.Exists(partFilename))
                    {
                        try
                        {
                            ...
                            LogInfo($"Deleted {partFilename}");
                        }
                        catch (Exception ex)
                        {
                            LogError($"Failed to delete {partFilename}: {ex.Message}");
                        }
                    }
                }
            }
